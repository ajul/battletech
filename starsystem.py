...
result_image = Image.new('RGB', (output_size_x, output_size_y))
result_draw = ImageDraw.Draw(result_image)

result_imagemap = '<imagemap>\n'
result_imagemap += 'Image:Starsystem.png' # add other image stuff here
...
for data in datas:
    ...
    color = faction_colors[data["Owner"]]
    
    result_draw.ellipse([
        x - star_radius,
        y - star_radius,
        x + star_radius,
        y + star_radius], fill=color)
    ...
    result_draw.text((text_x, text_y), name, font=font, fill=color)

    result_imagemap += 'circle %d %d %d [[%s]]\n' % (x, y, imagemap_radius, name)

result_image.save('starsystem.png')
